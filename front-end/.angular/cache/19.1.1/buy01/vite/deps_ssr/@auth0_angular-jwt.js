...
var fromPromiseOrValue = (input) => {
  if (input instanceof Promise) {
    return (0, import_rxjs.defer)(() => input);
  }
  return (0, import_rxjs.of)(input);
};
var JwtInterceptor = class {
  ...
  handleInterception(token, request, next) {
    ...
    let tokenIsExpired = (0, import_rxjs.of)(false);
    if (this.skipWhenExpired) {
      tokenIsExpired = token ? fromPromiseOrValue(this.jwtHelper.isTokenExpired(token)) : (0, import_rxjs.of)(true);
    }
    if (token) {
      return tokenIsExpired.pipe((0, import_operators.map)((isExpired) => isExpired && this.skipWhenExpired ? request.clone() : request.clone({
        setHeaders: {
          [this.headerName]: `${authScheme}${token}`
        }
      })), (0, import_operators.mergeMap)((innerRequest) => next.handle(innerRequest)));
    }
  ...
  intercept(request, next) {
    ...
    return fromPromiseOrValue(token).pipe((0, import_operators.mergeMap)((asyncToken) => {
